...
class NPFLD(mx.gluon.HybridBlock):
    def __init__(self, num_of_pts=98, alpha=1.0, **kwargs):
        super(NPFLD, self).__init__(**kwargs)
        ...
        self.s2_conv = nn.Conv2D(channels=32, kernel_size=(3,3), strides=(2,2), padding=(1,1), activation=None, use_bias=False)
        self.s2_bn   = nn.BatchNorm(scale=True)
        self.s2_act  = nn.Activation('relu')
        self.s3_conv = nn.Conv2D(channels=128, kernel_size=(3,3), strides=(2,2), padding=(1,1), activation=None, use_bias=False)
        self.s3_bn   = nn.BatchNorm(scale=True)
        self.s3_act  = nn.Activation('relu')
        
        self.s1_flatten = nn.Flatten()
        self.s2_flatten = nn.Flatten()
        self.s3_flatten = nn.Flatten()

        self.lmks_out = nn.HybridSequential()
        self.lmks_out.add(
            nn.Dense(units=self.pts_num*2, activation=None, use_bias=True)
        )
    ...
    def hybrid_forward(self, F, x):
        ...
        s1  = self.s1_flatten(s1)
        s2  = self.s2_flatten(s2)
        s3  = self.s3_flatten(s3) 
...
if __name__ == '__main__':
	...
	net = NPFLD(num_of_pts=98, alpha=0.25)
